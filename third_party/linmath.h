...
static inline void vec##n##_mul_elem(vec##n r, vec##n const a, vec##n const b) \
{ \
        int i; \
        for(i=0; i<n; ++i) \
                r[i] = a[i] * b[i]; \
...
static inline void mat4x4_transpose(mat4x4 M, const mat4x4 N)
{
	int i, j;
	for(j=0; j<4; ++j)
		for(i=0; i<4; ++i)
...
static inline void mat4x4_scale_aniso(mat4x4 M, const mat4x4 a, float x, float y, float z)
{
	int i;
...
static inline void mat4x4_rotate_X(mat4x4 Q, const mat4x4 M, const float angle)
{
	float s = sinf(angle);
	float c = cosf(angle);
	mat4x4 R = {
...
static inline void mat4x4_rotate_Y(mat4x4 Q, const mat4x4 M, float angle)
{
	float s = sinf(angle);
	float c = cosf(angle);
	mat4x4 R = {
...
static inline void mat4x4_rotate_Z(mat4x4 Q, const mat4x4 M, float angle)
{
	float s = sinf(angle);
	float c = cosf(angle);
	mat4x4 R = {
...
static inline void mat4x4_orthonormalize(mat4x4 R, const mat4x4 M)
{
	float s = 1.;
	vec3 h;
...
static inline void quat_sub(quat r, const quat a, const quat b)
{
	int i;
	for(i=0; i<4; ++i)
		r[i] = a[i] - b[i];
...
static inline float quat_inner_product(quat a, const quat b)
{
	float p = 0.f;
	int i;
	for(i=0; i<4; ++i)
...
static inline void quat_rotate(quat r, float angle, const vec3 axis) {
	int i;
	vec3 v;
...
static inline void mat4x4o_mul_quat(mat4x4 R, const mat4x4 M, const quat q)
{
